...
public class HashTableWithSeperateChaining<K, V> implements Iterable<K> {
	  ...
	  public HashTableWithSeperateChaining(int capacity) {
		    this(capacity, DefaultLoadFactor);
	}

    // constructor
	  public HashTableWithSeperateChaining(int capacity, double LoadFactor) {
		//Capacity can't be less than zero 
		  if (capacity < 0) throw new IllegalArgumentException("must be a postive number for  capacity"); 
		  //check if load factor is less than zero, if its not a number, and some huge integer
		  if (LoadFactor <= 0 || Double.isNaN(LoadFactor) || Double.isInfinite(LoadFactor))
		    throw new IllegalArgumentException(" LoadFactor cannot be a negative value");
		    this.LoadFactor = LoadFactor;
		    this.capacity = Math.max(DefaultCapacity, capacity); //returns the greater of two int values. That is, the result is the argument closer to positive infinity.
	 ...
	 // Returns the size
	  ...
	  // delete contents of the table, fill the table with null values and reset the size to 0 
	  public void clear() {
	  ...
	  /*
	   * Everytime we seek something within the bucket be it a function such as hasKey we need create a bucket index to look at
	   * the values 
	   *
	   * A hash table that uses buckets is actually a combination of an array and a linked list. Each
         ...
         In case a bucket is completely full, the record will get stored in an overflow bucket of infinite capacity at 
         the end of the table. All buckets share the same overflow bucket. However, a good implementation will use a 
         hash function that distributes the records evenly among the buckets so that as few records as possible go into 
         the overflow bucket.
	   * 
	   */
	  ...
	  public V remove(K key) {
		  ...
		  /*
		   * Bucket remove, bucket seek and bucket insert all work with a bucket index, we need to first find the place
		   * of the value within the buckets 
		   */
